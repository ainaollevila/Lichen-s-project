...
#do
#    cat "$file" |  sed "s/\s*//g" | awk 'BEGIN{FS=":"; varname="" ; varvalue=""}{if($2 != ""){varname=$1","varname; }}END{print varname "mutualism,ticks,sexproba,rep,pop"}' > "$outputname"
#    break 1
#done
...
do
	...
	do
		...
		for pop in {1..100}; 
		do
		    filename=mutualism_"$mutualism"_"$ticks"ticks_"$sexprob"sexualreproduction_replicate"$rep".dat"$pop"_"$suffix".txt
		    file="$foldername"/"$filename"
		    if [ -f $file ];
		    then
			echo "Parsing file:$file"
			cat  $file | sed "s/\s*//g" | awk -v pop="$pop" -v mut="$mutualism" -v ticks="$ticks" -v sp="$sexprob" -v rep="$rep" 'BEGIN{FS=":"; varname="" ; varvalue=""}{if($1 == "string"){exit}if($2 != ""){varname=$1","varname; varvalue=$2","varvalue}}END{print varvalue mut","ticks","sp","rep","pop}' >> "$outputname"
		    fi
		done
	    done
	done
